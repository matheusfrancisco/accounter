...
describe('HolderController', () => {
    ...
    class HolderServiceMock extends HolderService {
        constructor(holderRepository: HolderRepository, createHolder: (name: string, taxpayerRegistry: string) => Promise<void>) {
            super(holderRepository);
    ...
    beforeEach(() => {
        ...
        holderController = buildHolderController(new HolderServiceMock(holderRepository, createHolder));
        failingHolderController = buildHolderController(new HolderServiceMock(holderRepository, () => { throw new ServiceError('Service error'); }));
        failingForUnknownReasonsHolderController = buildHolderController(new HolderServiceMock(holderRepository, () => { throw new Error('Unknown error'); }));
        end = sinon.spy();
        json = sinon.spy();
    ...
    it('Should throw if error is unknown', () => {
        expect(failingForUnknownReasonsHolderController.post(req, resMock))
            .to.eventually.throw;
